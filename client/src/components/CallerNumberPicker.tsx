...
export default function CallerNumberPicker({ value, onChange }: { value: string; onChange: (from: string) => void }) {
  ...
  const box: React.CSSProperties = { border: '1px solid var(--line)', borderRadius: 10, padding: 14, marginBottom: 10 };
  const radio = (m: 'list' | 'own', title: string, sub: string) => (
    <label style={{ ...box, display: 'flex', gap: 10, cursor: 'pointer', borderColor: mode === m ? 'var(--cyan)' : 'var(--line)' }}>
      <input type="radio" checked={mode === m} onChange={() => setMode(m)} />
      <span>
        <div style={{ fontSize: 13.5, fontWeight: 700, color: 'var(--tx)' }}>{title}</div>
  ...
  return (
    <div>
      <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--tx)', marginBottom: 8 }}>Call from</div>
      ...
      {mode === 'own' && (
        <div style={box}>
          {phase !== 'verified' && (
            <>
              <input value={ownNumber} onChange={(e) => setOwnNumber(e.target.value)} placeholder="+919876543210"
                style={{ width: '100%', padding: '10px 12px', background: 'var(--bg-secondary)', border: '1px solid var(--line)', borderRadius: 8, color: 'var(--tx)', fontSize: 13, marginBottom: 8 }} />
              <button onClick={startVerify} disabled={phase === 'calling'}
                style={{ padding: '9px 18px', borderRadius: 8, border: '1px solid var(--cyan)', background: 'transparent', color: 'var(--cyan-fg)', cursor: 'pointer', fontSize: 13 }}>
                {phase === 'calling' ? 'Waiting for you to answer…' : 'Verify this number'}
              </button>
            </>
          )}
          {code && phase === 'calling' && (
            <div style={{ marginTop: 10, fontSize: 13, color: 'var(--tx-2)', lineHeight: 1.6 }}>
              📞 We're calling <b>{ownNumber}</b> now. Answer and type this code on your keypad:
              <div style={{ fontSize: 26, letterSpacing: 8, fontWeight: 800, color: 'var(--cyan-fg)', margin: '6px 0' }}>{code}</div>
          ...
          <div style={{ marginTop: 12, fontSize: 12, color: 'var(--tx-3)', lineHeight: 1.55, borderTop: '1px solid var(--line)', paddingTop: 10 }}>
            ...
            <a href="/airtel-verified-calling" target="_blank" rel="noreferrer" style={{ color: 'var(--cyan-fg)' }}>Step-by-step guide →</a>
          </div>
        </div>
      )}
    </div>
