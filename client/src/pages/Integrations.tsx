...
function ConnectModal({ provider, oauthAvailable, onClose, onConnected }: {
  ...
  const handleOAuthConnect = async () => {
    ...
    try {
      ...
      // app credentials aren't configured server-side — say exactly that
      // instead of silently degrading to a confusing custom-webhook form.
      throw new Error(
        'One-click OAuth for this platform is not configured on the server yet (its OAuth app credentials are missing from backend/.env). You can connect manually with an API token below, or ask your admin to configure the platform OAuth app.'
      );
    } catch (e: any) {
...
export default function Integrations() {
  ...
  useEffect(() => {
    ...
    if (err) {
      ...
      setCallbackState({ type: 'error', provider: provider ?? undefined, message });
      toast.error(message);
    }
    if (connected || err) window.history.replaceState({}, '', window.location.pathname);
  }, []);

  // SSE live updates — authenticated via header (fetch-based SSE); workspaceId
  // is recovered from the JWT if missing from storage, so live updates attach
  // even on a fresh session (previously bailed out silently).
  useEffect(() => {
    const handle = integrationsApi.subscribeEvents(() => load(true));
    return () => handle?.close();
  }, []);
