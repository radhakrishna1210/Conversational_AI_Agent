...
const heroPoints = [
  {
    ...
    description: 'Guide prospects through applications, deadlines, and requirements',
  },
  {
    title: 'Financial Aid',
    description: 'Explain FAFSA, scholarships, and payment options clearly',
  },
  {
    title: 'Campus Tours',
    description: 'Schedule visits, coordinate tours, and answer questions',
  },
];

const educationTemplates: Array<{ label: string; icon: LucideIcon }> = [
  { label: 'Enrollment Inquiry Handler', icon: GraduationCap },
  { label: 'Course Information Assistant', icon: FileText },
  { label: 'Application Status Follow-Up', icon: CalendarCheck },
  { label: 'Enrollment Reminder Calls', icon: Bell },
  { label: 'Event Invitation Calls', icon: PhoneCall },
];
...
const featureCards = [
  {
    ...
    description:
      'Help students register for classes and build their schedules. Check prerequisites, resolve conflicts, and manage waitlists.',
    tags: ['Schedule building', 'Conflict resolution', 'Prerequisite checking', 'Waitlist management'],
  },
  {
    icon: MapPin,
    title: 'Campus Tour Scheduling',
    description:
      'Book campus visits and answer questions about student life. Coordinate tours, department visits, and open houses.',
    tags: ['Tour booking', 'Department visits', 'Event coordination', 'Campus information'],
  },
];

const faqs = [
  {
    question: 'What can OmniDimension automate for educational institutions?',
    answer:
  ...
  {
    question: 'Can it handle admission calls and follow-ups?',
    answer:
      'Yes. The assistant can answer admissions questions, collect basic applicant details, and follow up with reminders or next steps based on your workflow.',
  },
  {
    question: 'Does it support multilingual communication with parents?',
    answer:
      'Yes. You can configure multilingual conversations so parents and students can interact in the language they prefer.',
  },
  {
    question: 'Can it make bulk calls and integrate with our systems?',
    answer:
      'The assistant can support outreach workflows and connect to your CRM, SIS, or other systems through integrations and API-based automation.',
  },
];
          ...
          <div className="grid gap-10 lg:grid-cols-[1.04fr_0.96fr] lg:gap-12">
            ...
            >
              ...
              <p className="mt-7 max-w-[700px] text-[18px] leading-[1.75] text-gray-700 dark:text-white/70">
                Handle admissions, enrollment, financial aid, and campus tours with intelligent voice assistants.
                Support students 24/7 while your team focuses on education.
              </p>
        ...
        <section className="relative mx-auto max-w-[1480px] px-6 pb-24 lg:px-10 lg:pb-28">
          ...
          <div className="mx-auto mt-14 max-w-[920px] space-y-4">
            {faqs.map((faq, index) => {
              ...
              const questionId = `education-faq-question-${index}`;
              const answerId = `education-faq-answer-${index}`;
      ...
      <div className="mx-auto max-w-[1480px] px-6 lg:px-10">
        <div className="grid gap-14 lg:grid-cols-[0.9fr_1fr_1fr_1fr_1fr]">
          <div>
            ...
            <p className="mt-6 text-[18px] font-semibold text-gray-900 dark:text-white/92">Ask AI about OmniDimension</p>
        ...
        <div className="mt-24 flex flex-col gap-6 border-t border-gray-300 dark:border-white/6 pt-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="text-[14px] text-gray-500 dark:text-white/42">© 2026 OmniDimension</div>
...
function FooterIconLabel({ label }: { label: string }) {
  return (
    <div className="flex h-10 w-10 items-center justify-center rounded-[10px] border border-gray-300 dark:border-white/10 bg-gray-100 dark:bg-[#171717] text-[14px] text-gray-600 dark:text-white/72 transition-colors">
      {label}
    </div>
  );
}
...
function FooterColumn({ heading, items }: { heading: string; items: string[] }) {
  return (
    <div>
      <div className="text-[16px] font-semibold text-gray-900 dark:text-white/88">{heading}</div>
      <div className="mt-7 space-y-4 text-[15px] text-gray-600 dark:text-white/50 transition-colors">
        {items.map((item) => (
          <div key={item} className={item === 'BY INDUSTRY' || item === 'BY USE CASE' || item === 'LEGAL' ? 'mt-2 text-[13px] font-semibold uppercase tracking-[0.18em] text-gray-500 dark:text-white/72 transition-colors' : ''}>
            {item}
          </div>
        ))}
      </div>
