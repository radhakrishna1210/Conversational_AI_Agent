...
function VideoCard({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <div style={{ border: '1px solid var(--line)', borderRadius: 10, overflow: 'hidden', cursor: 'pointer', width:'100%', maxWidth:320, transition: 'border-color 0.2s' }}
    ...
    >
      <div style={{ width: '100%', aspectRatio: '16/9', background: 'linear-gradient(135deg, #0d1525 0%, #162136 50%, #1a2740 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' as const, overflow: 'hidden' }}>
        <div style={{ position: 'absolute' as const, inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' as const, gap: 4 }}>
          <div style={{ fontSize: 10, color: 'var(--tx-2)', fontWeight: 700, letterSpacing: '0.05em', textTransform: 'uppercase' as const }}>VIDEO</div>
        ...
        <div style={{ position: 'absolute' as const, bottom: 0, left: 0, right: 0, background: 'linear-gradient(0deg, rgba(0,0,0,0.6) 0%, transparent 100%)', padding: '20px 10px 10px', fontSize: 11, fontWeight: 700, color: '#ffffff', lineHeight: 1.4 }}>
          {title}
        </div>
      </div>
      <div style={{ padding: '12px 14px' }}>
        {subtitle && <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--tx)', marginBottom: 6, lineHeight: 1.4 }}>{subtitle}</div>}
      ...
      <div style={{ width:'100%', maxWidth:760, margin:'0 auto', padding:'clamp(24px,5vw,48px) clamp(16px,4vw,24px) 80px', boxSizing:'border-box' }}>
        ...
        <div style={{ display:'flex', flexWrap:'wrap', alignItems:'center', gap:20, marginBottom:32 }}>
          ...
          <div style={{ paddingTop: 4 }}>
            <div style={{ fontSize:'clamp(24px,5vw,28px)', fontWeight: 800, color: 'var(--tx)', marginBottom: 4, letterSpacing: '-0.5px' }}>Custom API</div>
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))', gap: 20 }}>
            <div>
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--tx)', marginBottom: 4 }}>Webhook</div>
            ...
            <div>
              ...
              <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--tx)', marginBottom: 4 }}>Web Search</div>
              <div style={{ fontSize: 12, color: 'var(--tx-2)', lineHeight: 1.6 }}>
