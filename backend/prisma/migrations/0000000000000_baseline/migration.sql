-- Baseline: the full schema as it stood on 2026-09-03 (before CallTransfer),
-- generated with `prisma migrate diff --from-empty --to-schema-datamodel`.
-- The 17 incremental migrations that used to precede it are in
-- prisma/migrations_archive/ (history only; their rows stay in
-- _prisma_migrations on existing databases as harmless orphans).
-- On an EXISTING database run `prisma migrate resolve --applied 0000000000000_baseline` ONCE before the next deploy.
