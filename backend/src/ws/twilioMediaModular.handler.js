...
export function handleTwilioMediaModularUpgrade(ws, { workspaceId, agentId }) {
  ...
  const finalizeCallLog = createCallFinalizer({
    workspaceId, agentId, label: 'modular phone call',
  });
    ...
    switch (msg.event) {
      ...
      case 'start': {
        ...
        try {
          ...
          logger.error(`Failed to start modular phone bridge: ${err.message}`);
          cleanup('FAILED');
          ws.close();
        }
        break;
  ...
  ws.on('error', (err) => {
    logger.warn(`Modular phone media socket error: ${err.message}`);
    cleanup('FAILED');
  });
}
