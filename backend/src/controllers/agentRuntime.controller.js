...
import { speculationModeFor, SPECULATION_MODES } from '../services/voice/speculativeTurn.js';
import { resolveTransferConfig } from '../services/telephony/transfer.service.js';
import { resolveAmbientMode, AMBIENT_MODES, ALL_AMBIENT_PRESET_NAMES } from '../services/voice/ambience.js';
  ...
  try {
    ...
    res.json({
      ...
      ttsDelivery: {
        ...
        // Without the provider: clients are never told who speaks their voice,
        // and the capability reasons name it.
        voice: {
          tokenStreaming: capabilities.tokenStreaming,
          ssmlBreaks: capabilities.ssmlBreaks,
          deliveryMode: capabilities.deliveryMode,
          reason: capabilities.tokenStreaming
            ? null
            : (voice ? 'Replies in this voice are synthesized sentence by sentence.' : capabilities.reason),
        },
      },
      ambience: {
        mode: resolveAmbientMode(settings),
        modes: AMBIENT_MODES,
        presets: ALL_AMBIENT_PRESET_NAMES,
        // Native (Fish-generated) ambience needs a Fish voice on an S2 model.
        nativeAvailable: capabilities.providerName === 'FishAudio' && /^s2/.test(process.env.FISH_TTS_MODEL || 's2.1-pro'),
        // Worded without the provider, for the same reason as above.
        nativeReason: capabilities.providerName === 'FishAudio'
          ? (/^s2/.test(process.env.FISH_TTS_MODEL || 's2.1-pro') ? null : 'Voice-generated background sound is not available on this platform right now.')
          : (voice
            ? "Voice-generated background sound needs a voice that supports it, and this agent's voice does not."
            : 'Choose a voice for this agent first.'),
        manualAvailable: process.env.AMBIENCE_PHONE_ENABLED !== 'false',
      },
      speculation: {
        selected: speculationModeFor(settings),
        modes: SPECULATION_MODES,
        available: process.env.VOICE_SPECULATION !== 'off',
      },
      transfer: (() => {
        const cfg = resolveTransferConfig(settings);
        return {
          configured: cfg.enabled,
          number: cfg.number,
          mode: cfg.mode,
          timeoutSec: cfg.timeoutSec,
          outOfHours: cfg.outOfHours,
          hours: cfg.hours,
          // Which call paths can actually hand over. A browser call never can.
          carriers: { TWILIO: true, PLIVO: true, PIOPIY: false, WEB: false },
        };
      })(),
    });
  } catch (err) {
