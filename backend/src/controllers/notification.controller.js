...
export const markAsRead = async (req, res) => {
  ...
  try {
    // `workspaceId` is not part of the unique selector on Notification, so a
    // compound `where: { id, workspaceId }` in update() is invalid Prisma and
    // throws at runtime. updateMany supports arbitrary filters and keeps the
    // operation workspace-scoped (a user can never touch another workspace's rows).
...
export const deleteNotification = async (req, res) => {
  ...
  try {
    const result = await prisma.notification.deleteMany({ where: { id, workspaceId } });
